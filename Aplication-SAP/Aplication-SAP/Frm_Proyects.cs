...
namespace Aplication_SAP
{
    public partial class Frm_Proyects : Form
    {
        ...
        //Cambio se debe para Git actuaizar
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            if (this.txtCodeProyecto.Text != string.Empty)
            {
                //Verificamos que si exista el producto
                if (this.metodosProyectos.searchCode(this.txtCodeProyecto.Text) == true)
                {
                    this.completeDataProject(this.txtCodeProyecto.Text.Trim());
                }//Fin del if
                else
                {
                    MessageBox.Show("El Código ingresado no se encuentra registrado en el sistema", "Código inexistente", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            {
                MessageBox.Show("Favor ingrese un valor valido de codigo de proyecto", "Espacios vacíos", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }//fin condicion espacios vacios
        }// Fin de método

        private void completeDataProject(string code)
        {
            try//Manejamos un try para controlar un errror durante la utilización del método de búsqueda
            {
                //Asignar valores de data set
                DataSet dataProject = this.metodosProyectos.projectData(code);
                //Recorrer valores de las columnas del dataSet y asignarlas
                foreach (DataRow dr in dataProject.Tables[0].Rows)
                {
                    this.txtNombreProyecto.Text = dr[1].ToString();
                    this.txtDescripcion.Text = dr[2].ToString();
                    this.dateTimeStart.Value = Convert.ToDateTime(dr[3]);
                    this.dateTimeEnd.Value = Convert.ToDateTime(dr[4]);
                }
